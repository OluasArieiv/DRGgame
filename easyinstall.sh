...
curl -s -L https://raw.githubusercontent.com/OluasArieiv/DRGgame/refs/heads/master/LANGUAGE.po > $HOME/twm/LANGUAGE.po
content=$(cat "$HOME/twm/LANGUAGE.po")

if [ ! -e "~/twm/info.sh" ]; then
 curl curl https://raw.githubusercontent.com/OluasArieiv/DRGgame/master/info.lib -s -L >$HOME/twm/info.lib
...
if uname|grep -q -i "cygwin"; then
 ...
 if [ -e /bin/apt-cyg ]; then
  ...
  #/cygwin repository
  curl -s -L -O "https://raw.githubusercontent.com/OluasArieiv/DRGgame/master/apt-cyg" &>/dev/null
  install apt-cyg /bin
 fi
...
sync_func_other () {
 ...
 curl ${SERVER}twm.sh -s -L|sed -n '2,40p' >>twm.sh
 NUM_SCRIPTS=$(echo $SCRIPTS|wc -w)
 LEN=0
 ...
 curl ${SERVER}twm.sh -s -L|sed -n '47,94p' >>twm.sh
